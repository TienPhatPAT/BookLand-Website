...
const Billing = () => {
  ...
  useEffect(() => {
    fetchApi(`${getApiEnv()}/order`).then((data) => {
          ...
          {orderList.map((order) => (
            ...
            >
              <div className={classes.img}>
                <img src={order.items[0].book.img} alt={order.items[0].book.ten} />
              </div>
              <div className={classes.info}>
                <div className={classes.name_author}>
                  <Typography variant="h6">{order.items[0].book.ten}</Typography>
                  <Typography variant="body2">{`author`}</Typography>
                </div>
                <div className={classes.price_amount}>
                  <Typography variant="h6">{`${formatMoney(
                    order.items[0].book.gia
                  )} VNĐ`}</Typography>
